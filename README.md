...
> **Codex / CC Switch 特别说明（透明中转模式）**：Codex 必须把 `base_url` 指向 `http://127.0.0.1:57399/v1`，Bridge 的 `upstream` 再指向 CC Switch（通常是 `http://127.0.0.1:15721`）。如果 Codex 直接指向 `15721`，CC Switch 会在图片到达 Bridge 之前以“当前模型不支持 vision”拒绝请求。Bridge 启动后会监听 `config.toml`，CC Switch 切换供应商覆盖 `base_url` 时自动恢复 Bridge 入口；切换供应商可能重启路由并导致当前会话断开，请等待恢复后再重试，必要时重启 Codex。
...
- **Codex / cc-switch 用户**：Bridge 启动时会**自动检测并修复**模型目录（把所有模型的 `input_modalities` 动态补上 `image`，不写死任何模型名）；用 cc-switch 切换模型导致目录被覆盖时，也会自动重新修复。Bridge 同时会把 Codex `base_url` 保持在 `57399`，避免 CC Switch 的 `15721` 提前拦截图片。修复后**重启客户端**即可发图。
