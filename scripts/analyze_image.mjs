...
const apiKey = flag("--api-key") || process.env.VISION_API_KEY || cfg.apiKey || cfg.visionApiKey || cfg.zhipuApiKey || "";
...
const provider = getProvider(cfg.provider || cfg.visionProvider || "");
const providerStyle = provider?.style || "openai";
const accountId = cfg.accountId || cfg.visionAccountId || "";

function finalBaseUrl() {
  let url = baseUrl;
...
if (!apiKey && !provider?.local) fail("未配置视觉 API Key。运行 node scripts/setup.mjs，或设置 VISION_API_KEY。");
...
async function toDataUri(input) {
  if (/^data:/i.test(input)) {
    const base64 = input.includes(";base64,") ? input.split(";base64,")[1] : "";
    if (base64 && Buffer.from(base64.replace(/\s+/g, ""), "base64").length > MAX_IMAGE_BYTES) {
...
let requestUrl = finalBaseUrl();

if (providerStyle === "gemini") {
  const m = /^data:([^;,]+);base64,(.*)$/s.exec(dataUri);
  if (!m) throw new Error("Gemini 需要 base64 data URL");
  body = {
    contents: [{
      role: "user",
      parts: [
...
} else if (providerStyle === "cloudflare") {
